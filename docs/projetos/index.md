# Projetos

Os projetos são independentes e podem ser feitos em qualquer ordem, mas a sequência abaixo introduz os conceitos de forma gradual.

| # | Projeto | Conceitos | Duração |
| --- | --- | --- | --- |
| 1 | [Jogo da Memória](01-jogo-da-memoria/index.md) | conceitos | duracao |
| 2 | [Radar Ultrassônico](02-radar-ultrassonico/index.md) | conceitos | duracao |

## Como cada projeto está organizado

Todo projeto segue a mesma estrutura de três páginas:

- **Visão geral** — o que o projeto faz e a lista de materiais.
- **Montagem** — o circuito, pino a pino.
- **Firmware** — o código comentado e como gravá-lo na placa.
